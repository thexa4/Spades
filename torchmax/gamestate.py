...
class GameState:
    ...
    def input(self):
        ...
        parts.append(np.array(self.bid_state['bids'], dtype=np.float32).flatten())
        parts.append(np.array(self.bid_state['hand'], dtype=np.float32).flatten())
        parts.append(np.array(self.bid_state['bags'], dtype=np.float32).flatten())

        for i in range(13):
            roundname = 'round' + str(i) + '_'

            parts.append(np.array(rounds[i]['seen'], dtype=np.float32).flatten())
            parts.append(np.array(rounds[i]['hand'], dtype=np.float32).flatten())
            parts.append(np.array(rounds[i]['played'], dtype=np.float32).flatten())
            parts.append(np.array(rounds[i]['todo'], dtype=np.float32).flatten())
    ...
    def training_data(self):
        ...
        parts.append(np.array(self.thermometer(14, self.chosen_bid), dtype=np.float32))

        for i in range(13):
            parts.append(np.array(self.thermometer(52, self.chosen_cards[i]), dtype=np.float32))

        return np.concatenate(parts)

    def store_game(self, score):
        return np.concatenate([self.training_data(), np.array([score])], dtype=np.float32)